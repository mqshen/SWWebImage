...
class ViewController: UITableViewController {
    ...
    let diskLabel = UILabel(frame: CGRectMake(10, 40, 300, 20))
    ...
    override func viewDidLoad() {
        ...
        let headView = UIView(frame: CGRectMake(0, 0, 320, 120))
        
        headView.addSubview(diskLabel)
        
        let refreshButton = UIButton(frame: CGRectMake(100, 80, 80, 20))
        refreshButton.addTarget(self, action: "doRefresh", forControlEvents: UIControlEvents.TouchUpInside)
        refreshButton.setTitle("refresh", forState: UIControlState.Normal)
        refreshButton.setTitleColor(UIColor.blackColor(), forState: UIControlState.Normal)
        headView.addSubview(refreshButton)
        
        let cleanButton = UIButton(frame: CGRectMake(200, 80, 80, 20))
        cleanButton.addTarget(self, action: "doClean", forControlEvents: UIControlEvents.TouchUpInside)
        cleanButton.setTitle("clean", forState: UIControlState.Normal)
        cleanButton.setTitleColor(UIColor.blackColor(), forState: UIControlState.Normal)
    ...
    func doRefresh() {
        let diskSize = SWWebImageManager.sharedManager.imageCache.getSize()
        
        diskLabel.text = "disk size: \(diskSize)"
    }
    
    func doClean() {
        SWWebImageManager.sharedManager.imageCache.cleanDiskWithCompletionBlock { () -> Void in
            self.doRefresh()
        }
    }
